...
class DocumentPrepareResponse(BaseModel):
    """Response with document metadata and processing costs."""  # TODO describe response wtih `Args:`

    hash: str
    metadata: DocumentMetadata
    endpoint: Literal["text", "website", "document"]
    credit_cost: int | None = (
        None  # TODO: Implement credit cost calculation should it be just dumb estimate (pagecount * credit per page for the selected processing method) or should we check if the requested pages are already cached and only charge for the new ones? Prlly the later, shouldnt be any additional burden, if we refactor a util function from the document processor. We just need to check the cached object, no more.
    )
...
async def prepare_document(
    ...
    if cached_data:
        ...
        return DocumentPrepareResponse(hash=cache_key, metadata=cached_doc.metadata, endpoint=endpoint)
    ...
    return DocumentPrepareResponse(hash=cache_key, metadata=metadata, endpoint=endpoint)
...
async def prepare_document_upload(file: UploadFile, cache: DocumentCache) -> DocumentPrepareResponse:
    ...
    if cached_data:
        cached_doc = CachedDocument.model_validate_json(cached_data)
        return DocumentPrepareResponse(hash=cache_key, metadata=cached_doc.metadata, endpoint="document")
    ...
    return DocumentPrepareResponse(hash=cache_key, metadata=metadata, endpoint="document")
...
async def create_website_document(
    ...
    cached_doc = CachedDocument.model_validate_json(cached_data)
    # Verify this is actually a website
