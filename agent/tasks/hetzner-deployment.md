...
All services healthy, auth flow working end-to-end.
...
8. ~~Create Stack Auth project~~ ✓ (via auth.yaptts.org dashboard)
9. ~~Test end-to-end~~ ✓ (sign up works!)
...
**Traefik recovery:**
- Traefik was not running on prod (crashed earlier, wasn't recreated)
- Manually started with: `docker run -d --name traefik --restart=always -p 80:80 -p 443:443 -v /var/run/docker.sock:/var/run/docker.sock:ro -v /etc/dokploy/traefik:/etc/dokploy/traefik:rw --network dokploy-network traefik:v3.0 --configfile=/etc/dokploy/traefik/traefik.yml`
- If site goes down, first check: `docker ps | grep traefik`

**Nginx DNS caching issue:**
- After stack-auth container restart, nginx cached old IP (10.0.1.75 vs new 10.0.1.79)
- Fix: `docker exec yapit-test-app-nmqlyd-frontend-1 nginx -s reload`
- This happens because nginx resolves DNS at startup and caches it

**Dashboard access decision:**
- Initially chose SSH tunnel (localhost:18101)
- SSH tunnel failed: cookie issues (Partitioned attribute without Secure), CORS problems
- Switched to subdomain approach: auth.yaptts.org
- Dashboard and API now accessible via proper HTTPS subdomain

**Commits this session:**
- `c57a5a7` - fix: enable Stack Auth seed script for internal project creation
- `73f50db` - fix: Stack Auth routing - add dashboard route and fix API path
- `48983d0` - feat: upgrade Stack Auth to 3ef9cb3
- `57e7265` - fix: enable localhost for Stack Auth dashboard (SSH tunnel access)

### 2025-12-26 - Stack Auth Subdomain & Project Setup

**SSH tunnel approach failed:**
- Enabled `STACK_SEED_INTERNAL_PROJECT_ALLOW_LOCALHOST=true`
- User could access dashboard at localhost:18101 via SSH tunnel
- Sign-in succeeded but immediately redirected back to sign-in
- Root cause: Cookie with "Partitioned" attribute rejected without "Secure" attribute
- Dashboard at http://localhost makes API calls to https://yaptts.org → cookie domain mismatch

**Subdomain solution (auth.yaptts.org):**
- Added Traefik labels to stack-auth service with path-based routing:
  - Priority 20: `Host(auth.yaptts.org) && PathPrefix(/api)` → port 8102 (API)
  - Priority 10: `Host(auth.yaptts.org)` → port 8101 (Dashboard)
- Updated `NEXT_PUBLIC_STACK_API_URL` and `NEXT_PUBLIC_STACK_DASHBOARD_URL` to `https://auth.yaptts.org`
- DNS: Added A record `auth` → `78.46.242.1` (note: host should be just `auth`, not `auth.yaptts.org`)
- Let's Encrypt cert auto-generated by Traefik

**Important: DNS before deploy**
- DNS must propagate BEFORE deploying changes that reference the new domain
- Otherwise Let's Encrypt cert generation fails
- Check propagation: `dig auth.yaptts.org @8.8.8.8`
...
- `VITE_STACK_BASE_URL=https://auth.yaptts.org` (API endpoint)
- `VITE_STACK_AUTH_PROJECT_ID=553fe117-65ab-4f78-b37e-4ba2f54ca0cc`
- `VITE_STACK_AUTH_CLIENT_KEY=pck_...` (publishable, safe in git)
- These are baked into the Vite build - must rebuild frontend after changes

**Trusted domains (CRITICAL):**
- Stack Auth rejects requests from domains not in trusted list
- Error: `CURRENT_PROJECT_NOT_FOUND` even though project exists
- Must add `yaptts.org` via dashboard (Domains & Handlers tab)
- API doesn't expose domain management (requires "super secret admin key")
- One-time manual setup per project

**Nginx DNS caching:**
- After stack-auth container restart, nginx caches old IP
- Fix: `docker exec <frontend-container> nginx -s reload`
- Happens on every redeploy that restarts stack-auth

**Commits:**
- `fc07fc7` - feat: add auth.yaptts.org subdomain for Stack Auth dashboard
- `3ad8286` - chore: add Stack Auth project credentials to encrypted secrets
- `e6e0c08` - fix: add Stack Auth production credentials to frontend
