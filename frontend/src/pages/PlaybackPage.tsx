...
useEffect(() => {
  ...
  const processMessage = async () => {
    ...
    const floatData = new Float32Array(arrayBuffer); // or Int16Array if 16-bit PCM

    const channelCount = channels.current || 1;
    const frameCount = floatData.length / channelCount;

    const deinterleaved = Array.from({ length: channelCount }, (_, c) => new Float32Array(frameCount));
    for (let i = 0; i < frameCount; i++) {
      for (let c = 0; c < channelCount; c++) {
        deinterleaved[c][i] = floatData[i * channelCount + c];
      }
    }
