...
def _poll_status(gateway_url: str, doc_id: str, expect: str, timeout: float = 15.0) -> None:
    t0 = time.time()
    while time.time() - t0 < timeout:
        msg = requests.get(f"{gateway_url}/v1/documents/{doc_id}/filter_status").json()["message"]
...
def test_apply_filters_and_blocks(wait_until_gateway, gateway_url: str):
    ...
    _poll_status(gateway_url, doc_id, "done")

    # 5. verify blocks were rewritten and garbage removed
    blocks = requests.get(f"{gateway_url}/v1/documents/{doc_id}/blocks").json()["items"]
